...
- IAM ユーザー名: `terraform`（[terraform/iam/terraform-execution-policy.json](terraform/iam/terraform-execution-policy.json) を Customer Managed Policy として attach。inline は 2048 byte 上限を超えるため不可）
...
- **`terraform.tfvars` は gitignore 済み**。`allowed_cidrs` に自宅/オフィス IP を入れる用途で、誤コミット防止のため。
...
- 一連の手動構築完了後、GitHub Actions パイプライン化を予定（OIDC + IAM Role 構成見込み）。現時点では README に書かない方針。
