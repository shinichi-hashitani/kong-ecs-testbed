...
- [99-destroy.md](99-destroy.md) — 全環境抹消（連番 99 は将来 2 以降のシナリオを足す余地のため固定）

## 環境前提

- AWS リージョン: `ap-northeast-1`
- AWS 認証: 個人アカウントの IAM ユーザー Access Key を `aws configure --profile kong-testbed` で登録（saml2aws は使わない）
- IAM ユーザー名: `terraform`（[terraform/iam/terraform-execution-policy.json](terraform/iam/terraform-execution-policy.json) を Customer Managed Policy として attach。inline は 2048 byte 上限を超えるため不可。現状 4746 byte / 6144 上限）。GitOps 移行で OIDC Provider / S3 の権限が含まれている。policy 更新は権限昇格防止により terraform ユーザ自身では不可、IAM コンソール手動更新
...
0-setup.md / 1-gitops.md / 2-kong-upgrade.md / 99-destroy.md  # シナリオ別手順書
...
- **terraform/aws と terraform/konnect は backend "s3" {} 空ブロック**。`-backend-config` で bucket/key/region/use_lockfile を渡す partial config 形式（Terraform 1.10+ の S3 ネイティブロック使用、DynamoDB 不要）。bootstrap apply 後に `terraform init -migrate-state -backend-config=...` で local → S3 移行する。
