...
アップグレード前に、現在 ECS で稼働している DP のバージョンが既知（`3.13.x.x`）であることを確認する。`Server` ヘッダ・ECS Exec・Konnect コンソールの 3 系統で見て、ロールバックの基準点を握る。

```bash
set -a; source .env; set +a

# (1) Proxy 経路の Server ヘッダ（apikey 不要・最も手軽）
ALB_DNS=$(cd terraform/aws && terraform output -raw alb_dns_name)
curl -sI "http://${ALB_DNS}/httpbin/get" | grep -iE "^(server|x-kong)"
# => Server: kong/3.13.x.x-enterprise-edition
...
# => Kong Gateway 3.13.x.x ...

# (3) Konnect 側の DP 認識
# Konnect コンソール → Gateway Manager → kong-ecs-testbed-cp → Data Plane Nodes
# version カラムが 3.13.x.x になっていることを確認
```

> 💡 ECS Exec は AWS CLI の `session-manager-plugin` が要る。未インストールなら `brew install --cask session-manager-plugin` 後にリトライ。プラグイン無しでも (1) の `Server` ヘッダで version は取得できる。

> ✅ Step 1 完了条件:
> - `Server` ヘッダが `kong/3.13.x.x-...` を返す
> - Konnect の Data Plane Nodes で同じ version が見える

### Step 2. アップグレード対象のブランチを作成
...
### Step 3. `kong_dp_image` を 3.14 に更新
...
Step 1 と同じコマンドを実行し、`3.14.x.x` に切り替わっていることを確認する:

```bash
set -a; source .env; set +a

# (1) Proxy 経路の Server ヘッダ
ALB_DNS=$(cd terraform/aws && terraform output -raw alb_dns_name)
curl -sI "http://${ALB_DNS}/httpbin/get" | grep -iE "^(server|x-kong)"
# => Server: kong/3.14.x.x-enterprise-edition
...
# (4) Proxy が 200 を返すことを確認（DECK_API_KEY が .env にある場合のみ）
curl -i -H "apikey: $DECK_API_KEY" "http://${ALB_DNS}/httpbin/get"   # 200
```

> ✅ Step 7 完了条件:
> - `Server` ヘッダが `kong/3.14.x.x-...` を返す
> - Konnect の Data Plane Nodes 一覧で同じ version が見える
> - (DECK_API_KEY 設定時) `curl /httpbin/get` が 200
...
Step 1 / Step 7 と同じコマンドで、`3.13.x.x` に戻っていることを確認する:
...
# (3) Konnect 側の DP 認識
# Konnect コンソール → Gateway Manager → kong-ecs-testbed-cp → Data Plane Nodes
# version カラムが 3.13.x.x に戻っていることを確認
...
> ✅ ロールバック完了条件:
> - `Server` ヘッダが `kong/3.13.x.x-...` を返す
> - ECS service の最新 deployment が PRIMARY のみで running == desired
> - Konnect の Data Plane Nodes 一覧で version が `3.13.x.x` に戻っている
