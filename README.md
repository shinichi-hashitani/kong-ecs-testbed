...
```
.
├── README.md             # 本ファイル（Step 1〜7 の手順）
├── .env.example          # 環境変数テンプレート
├── .gitignore
├── terraform/
│   ├── iam/              # IAM 最小権限ポリシー JSON
│   ├── konnect/          # Konnect Control Plane (Step 2)
│   └── aws/              # VPC / ECS / ALB / IAM Roles (Step 3-5)
└── deck/                 # decK 設定 (Step 6)
```
...
クライアント → ALB → Kong Gateway DP → httpbin の経路で疎通する。Kong の Service / Route / Plugin 設定は decK 経由で Konnect CP に投入され、DP に配信される。
...
> 📅 将来的にはこのローカル Access Key 運用を廃止し、GitHub Actions から OIDC + IAM Role で同等のことを行う予定（別タスク）。
