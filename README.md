...
| 99 | [99-destroy.md](99-destroy.md) | 全環境を抹消する（AWS / Konnect / GitHub / ローカル） | 任意のタイミング |

各シナリオは前のシナリオの完了状態を前提とする。新規参加者は **0 → 1 → 2** の順で実行する。連番 99 は終端固定で、将来 `3-`〜`98-` 系のシナリオを追加する余地を残してある。
